...
var echarts_before = require('echarts/lib/echarts');
// 引入柱状图
...
export default class customeppqqrlist extends React.Component {
    ...
    loadData = (page, pageSize) => {
        ...
        $jsonp3(self, api.trouble4Map, {
            ...
            if (list.length > 0) {
                var showContent = <div>
                    海事部门:{list[0].msaName}<br/>分类：{list[0].sortName}<br/>检查项名称：{list[0].itemName}<br/>隐患等级：{list[0].grade == "2" ? "重大" : list[0].grade == "1" ? "一般" : "无隐患"}
                </div>;
                ...
                var showContent = "<div>海事部门：" + list[0].msaName + "<br/>检查人：" + list[0].userName + "<br/>分类：" + list[0].sortName + "<br/>检查项名称：" + list[0].itemName + "<br/>隐患等级：" + d1 + "<br/>问题描述：" + description + "<br/>处理措施：" + dealMeasure + "<br/>处理结果：" + dealResult + "<br/>限期天数：" + d2 + "</div>";
                self.setState({
                    markers: list,
                ...
                self.setState({
                    ...
                    console.log(self.state.centerlng)
                    console.log(self.state.centerlat)
                })
            }
    ...
    loadData1 = (page, pageSize) => {
        ...
        $jsonp3(self, api.troubleOfMsa, {
            ...
            var myChart1 = echarts_before.init(document.getElementById("mainAreaBar"), 'westeros');
            myChart1.setOption(option1);
            var myChart2 = echarts_before.init(document.getElementById("mainAreaPie"), 'westeros');
    ...
    loadData2 = (page, pageSize) => {
        ...
        $jsonp3(self, api.troubleOfMsaPreDay, {
            ...
            var myChart1 = echarts_before.init(document.getElementById("mainTimeLine"), 'westeros');
    ...
    clickit = (item, index) => {
        ...
        var showContent = "<div>海事部门：" + item.msaName + "<br/>检查人：" + item.userName + "<br/>分类：" + item.sortName + "<br/>检查项名称：" + item.itemName + "<br/>隐患等级：" + d1 + "<br/>问题描述：" + description + "<br/>处理措施：" + dealMeasure + "<br/>处理结果：" + dealResult + "<br/>限期天数：" + d2 + "</div>";
                        ...
                        <div style={{position: "relative"}}>
                            ...
                            {this.state.markers.length > 0 ?
                                ...
                                >
                                    ...
                                    <ScaleControl/>
                                    <OverviewMapControl/>
                                    <InfoWindow position={{lng: this.state.showInfoWindowlng,lat: this.state.showInfoWindowlat}} text={this.state.showInfoWindowContent} title={this.state.showInfoWindowTitle}/></Map>: ""}{this.state.markers.length == 0 ? <Map center={{lng: 120.613241,lat: 31.299671,}} style={{
                                                                       height: '700px',
                                                                       width: this.state.widthwidow,
                                                                       display: this.state.markers.length > 0 ? "none" : "block"
                                                                   }} enableScrollWheelZoom={true} zoom='13'>
                                <NavigationControl/>
                                <ScaleControl/>
                                <OverviewMapControl/>
