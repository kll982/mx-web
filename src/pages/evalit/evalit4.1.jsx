							...
							{this.state.QixianData.length>0?
								<div className={styles.tableMiddle2+" "+styles.flexTableColumn}>
									...
									{this.state.plainOptions1.map((item2,index)=>{
										return (
									<div className={styles.push1+" "+styles.flexFullItem}>
									<Input type="number" value={this.state["io-"+item.id+"-"+item1.id+"-"+item2.value]} ref={"io-"+item.id+"-"+item1.id+"-"+item2.value} disabled={this.state.disabled} name={"io-"+item.id+"-"+item1.id+"-"+item2.value}
							onChange={this.inputChange1.bind(this)} step="0.01"/></div>
										)
										
									})}
									<div className={styles.push1} style={{width:40}}>
									<Checkbox checked={this.state["check-"+item.id+"-"+item1.id]} onChange={this.onSelectChange1.bind(this)} ref={"check-"+item.id+"-"+item1.id} name={"check-"+item.id+"-"+item1.id} style={{display:this.state.isInConcat?"inline-block":"none"}}></Checkbox></div>
											</span>
